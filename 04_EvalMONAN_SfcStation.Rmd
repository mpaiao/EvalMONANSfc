...
knitr::opts_chunk$set(message=FALSE, results='hide', collapse = TRUE, warning = FALSE)
```
...
forecast_path = file.path(base_path,"SiteForecast")
...
* **format**. The file format used (e.g., `"csv"` for comma-separated values, `"tsv"` for tab-separated values, `"txt"` for (multi-)space-separated values).
* **accum_type**. How are accumulated variables reported? This is a character variable that takes the following options:

  - `"step"`. Variables are accumulated between two consecutive steps. 
  - `"daily"`. Variables are accumulated during the day, and reset at 00 UTC.
  - `"continuous"`. Variables are accumulated continuously, since the beginning of the simulation.
...
model_config = 
   tibble::tribble( ~name                   , ~desc                      , ~short                 , ~format,  ~accum_type, ~colour  , ~shape
                  , "MONAN-1.4.3rc_10km_00Z", "MONAN-1.4.3rc (10km, 00Z)","MONAN-1.4.3rc 10km 00Z",   "txt", "continuous", "#56BCC2",    15L
                  , "MONAN-1.4.3rc_10km_12Z", "MONAN-1.4.3rc (10km, 12Z)","MONAN-1.4.3rc 10km 12Z",   "txt", "continuous", "#E87D72",     5L
                  )#end tibble::tribble
```
...
* **add0**. A value $a$ to be added to both the **observed** and the **modelled** values so the results are in more friendly units. This implicitly assumes both models and observations have been pre-processed and all data are in the same standard units as MONAN. Check **mult** too. The complete operation from original variable $y_{o}$ to transformed variable $y_{t}$ will be $y_{t} = a + b \times y_{o}$. 
* **mult**. A multiplication factor $b$ to both the **observed** and the **modelled** values so the results are in more friendly units. This implicitly assumes both models and observations have been pre-processed and all data are in the same standard units as MONAN. Check **add0** too. The complete operation from original variable $y_{o}$ to transformed variable $y_{t}$ will be $y_{t} = a + b \times y_{o}$.
* **accum**. Is this variable an accumulated quantity? This is normally the case for variables such as rainfall and fluxes. If so, the variable will be harmonised, so the output is the cumulative sum at the same frequency for observations and model. 
* **unit**. The units to be displayed in non-normalised plots. This is a character, but it must comply with R mathematical language (check `help("plotmath")` for further information). This should be the unit after the transformation from **add0** and **mult** have been applied.
...
u_degC  = "degree*C"
u_MJom2 = "M*J^phantom(1)*m^{-2}"


varinfo = tibble::tribble( 
     ~name             , ~desc                           , ~short     ,   ~add0, ~mult, ~accum,   ~unit, ~assess,     ~colour  , ~cschm       , ~cnorm       ,      ~shape
   , "acswdnb"         , "Incoming SW irradiance"        , "Q[S*W]"   ,      0., 1.e-6,   TRUE, u_MJom2,    TRUE,     "#E31A1C", "magma"      , "i_RdGy"     ,         18L
   , "surface_pressure", "Surface pressure"              , "p[S*f*c]" ,      0.,  0.01,  FALSE,   u_hPa,    TRUE,     "#6A3D9A", "viridis"    , "i_PiYG"     ,         15L
   , "mslp"            , "Mean sea-level pressure"       , "p[S*L]"   ,      0.,  0.01,  FALSE,   u_hPa,    TRUE,     "#CAB2D6", "viridis"    , "i_PiYG"     ,          0L
   , "u10"             , "10-metre zonal wind speed"     , "v[10*m]"  ,      0.,    1.,  FALSE,   u_mos,   FALSE, NA_character_, NA_character_, NA_character_, NA_integer_
   , "v10"             , "10-metre meridional wind speed", "v[10*m]"  ,      0.,    1.,  FALSE,   u_mos,   FALSE, NA_character_, NA_character_, NA_character_, NA_integer_
   , "ws10"            , "10-metre wind speed"           , "v[10*m]"  ,      0.,    1.,  FALSE,   u_mos,    TRUE,     "#B2DF8A", "PuRd"       , "i_BrBG"     ,          8L
   , "t2m"             , "2-metre temperature"           , "T[2*m]"   , -273.15,    1.,  FALSE,  u_degC,    TRUE,     "#FB9A99", "plasma"     , "i_PuOr"     ,         17L
   , "td2m"            , "2-metre dew point temperature" , "T[d*2*m]" , -273.15,    1.,  FALSE,  u_degC,    TRUE,     "#A6CEE3", "YlGnBu"     , "RdBu"       ,          6L
   )#end tibble::tribble
```
...
Additional settings for the analyses and maps.
...
* **limit_time**. The first and last dates to include in the analysis, both in format "YYYY-MM-DD"
```{r, label='set-lonlatlim',message=FALSE,results='hide', collapse = TRUE}
limit_lon  = c(-85,-35)
limit_lat  = c(-60, 13)
limit_time = c("2025-12-01","2026-02-28")
```
...
cat0(" + Make a few global adjustments.")
...
cat0(" + Define R object containing the model assessment data.")
...
cat0(" + Verify which alternative distributions to fit.")
if (any(altDistr)){
...
cat0(" + Load administrative boundaries for countries and sub-national entities.")
...
cat0(" + Make sure all output paths exist.")
...
We also find the time boundaries for trimming the analyses. We add one day to the last element to trim the data at midnight.

```{r, label='set-time-bnds'}
limit_when     = lubridate::as_datetime(limit_time) + days(x=c(0,1))
```
...
cat0(" + Count the number of rows in each of the \"tibble\" objects.")
...
if (reload_rdata && file.exists(rdata_file)){
   cat0(" + Reload partially or fully pre-processed information from ",rdata_base,".")
   dummy = load(rdata_file)
}else{
   cat0(" + Start data pre-processing.")
   next_step = "load_info"
...
if (next_step %in% "load_info"){
   ...
   cat0(" + Read the climatic region shapes.")
   ...
   cat0(" + Save processed data to ",rdata_base,".")
   next_step = "load_site"
...
if (next_step %in% "load_site"){
   ...
   for (s in sequence(n_data_stream)){
      ...
      for (n in sequence(n_file_list)){
         ...
         if (nrow(s_data) == 0L ){
            cat0("   - File ",s_base," does not have sufficient data, skip processing.")
         }else{
            cat0("   - Process data from file ",s_base,".")
            ...
            # Trim data to the analysis period
            s_data = s_data %>%
               filter( ( when >= limit_when[1L] ) & ( when <= limit_when[2L] ) )
...
if (next_step %in% "load_model"){
   ...
   for ( m in sequence(n_model_config) ){
      ...
      m_accum_type = model_config$accum_type[m]
      ...
      for (n in sequence(n_file_list)){
         ...
         m_data = m_data                                                        %>%
            ...
            mutate( model = m_name, initial = min(when[timestep == 1L]))
         ...
         # Turn accumulated variables into accumulated between two consecutive steps.
         if (any(varinfo$accum) && m_accum_type %in% c("daily","continuous")){
            # List variables.
            v_accum = varinfo$name[varinfo$accum]

            # Decide how to reset variables.
            m_data = m_data %>%
               group_by(ident) %>%
               mutate(across(all_of(v_accum), ~ accum_one_step(.x,.data$when))) %>%
               ungroup()
         }#end if (any (varinfo$accum))


         # Select variables to keep.
         var_keep = c("ident","when","initial","timestep","model",varinfo$name[varinfo$assess])
         m_data = m_data %>%
            select(all_of(var_keep)) %>%
            filter( ( when >= limit_when[1L] ) & ( when <= limit_when[2L] ) )
...
In this step, we harmonise time and stations. We will keep only data where both model and observations are available, and we will make sure accumulated variables correspond to the same time interval for both data sets, so they are comparable.

```{r,label='harmon-time'}
# Temporarily reload script as I am working on
if (next_step %in% "harmon_time"){

   # Similarly, we keep only stations with data in both observations and forecasts.
   cat0(" + Keep only stations available at both observations and models (first pass).")
   ...
   # Find the times that exist for both observations and forecasts, then create an index that can be used to harmonise times.
   both_when    = sort(intersect(site_data$when,model_data$when))
   both_elapsed = as.numeric(both_when) - as.numeric(limit_when[1L])
   delta_step   = min(diff(both_elapsed))

   # For accumulated variables in models, we take the difference so the variables are not continuously accumulated.
   if (any(varinfo$accum)){
      # List variables for which harmonisation is required.
      v_accum = varinfo$name[varinfo$accum]
      site_data = site_data                                                                         %>%
         mutate( step = ceiling( ( as.numeric(when) - as.numeric(limit_when[1L]) ) / delta_step ) ) %>%
         group_by(ident, step)                   %>% 
         mutate(across(all_of(v_accum), ~ cumsum(.x))) %>%
         ungroup() %>%
         mutate( across(all_of(v_accum), ~ ifelse( test = step == 0L, yes = NA_real_, no =.x ) ) ) %>%
         select(! step)

      model_data = model_data %>%
         mutate( step = ceiling( ( as.numeric(when) - as.numeric(initial) ) / delta_step ) ) %>%
         group_by(ident, initial, step) %>% 
         mutate(across(all_of(v_accum), ~ cumsum(.x))) %>%
         ungroup() %>%
         mutate( across(all_of(v_accum), ~ ifelse( test = when == initial, yes = NA_real_, no =.x ) ) ) %>%
         select(! step)
   }#end if (any(varinfo$accum))

   cat0(" + Keep only times for which we check forecasts.")
   model_data = model_data %>%
      filter( timestep %in% forecastinfo$timestep)


   # Finally, we use pivot_longer ahead of merging.
   cat0(" + Re-organise data into longer tibbles and discard missing data rows.")
   site_data = site_data %>%
   ...
   cat0(" + Keep only times available at both observations and models (first pass).")
   both_when = sort(intersect(site_data$when,model_data$when))
...
}#end if (next_step %in% "harmon_time")
```
...
if (next_step %in% "fit_distr"){
   ...
   cat0(" + Find distribution statistics for observations (hang tight, this is an intensive step).")
   site_data = site_data %>%
      mutate( hour = hour(when) )
   ...
   cat0(" + Keep only entries with valid fitted statistics.")
   site_data = site_data %>%
      filter(! is.na(Distr))

   # Find the times that exist on both observations and forecasts and filter structures.
   cat0(" + Keep only times available at both observations and models.")
   both_when = sort(intersect(site_data$when,model_data$when))
   site_data  = site_data  %>% filter(when %in% both_when)
   model_data = model_data %>% filter(when %in% both_when)

   # Similarly, we keep only stations with data in both observations and forecasts.
   cat0(" + Keep only stations available at both observations and models (second pass).")
   both_ident = sort(intersect(site_data$ident,model_data$ident))
   site_data  = site_data  %>% filter(ident %in% both_ident)
   model_data = model_data %>% filter(ident %in% both_ident)
...
if (next_step %in% "merge_data"){
   ...
   cat0(" + Label time-variable combinations.")
   ...
   cat0(" + Merge observation and model.")
...
if (next_step %in% "find_goodness"){
   ...
   cat0(" + Summarise results for each region.")
...
cat0(" + Plot Taylor diagrams for each variable and region.")
...
for (v in var_loop){
   ...
   for (r in reg_loop){
      ...
      if (n_taylor_data > 0L){
         cat0("   - Variable: ",v_desc,". Region: ",r_desc,".")
...
cat0(" + Plot bias-variance diagrams for each variable and region.")
...
cat0(" + Plot Taylor diagrams for each variable and model.")
...
for (v in var_loop){
   ...
   for (m in model_loop){
      ...
      if (n_taylor_data > 0L){
         cat0("   - Variable: ",v_desc,". Model: ",m_desc,".")
...
cat0(" + Plot bias-variance diagrams for each variable and model.")
...
cat0(" + Plot Taylor diagrams for each model and region.")
...
for (m in model_loop){
   ...
   for (r in reg_loop){
      ...
      taylor_data = eval_data                                                %>%
         ...
         mutate( forecast = forecast_key[match(forecast,forecastinfo$name)]) %>%
         select(ident,lon,lat,alt,when,hour,forecast,variable,z_obs,z_mod)   %>%
      ...
      if (n_taylor_data > 0L){
         cat0("   - Model: ",m_desc,". Region: ",r_desc,".")
...
cat0(" + Plot bias-variance diagrams for each model and region.")
...
for (v in var_loop ){
   ...
   for (g in good_loop){
      ...
      for (f in sequence(n_forecastinfo)){
         ...
         if (v_plot){
            cat0(" + Plot map for ",g_desc," in ",v_desc,". Forecast: ",f_name,".")
