...
This R Markdown helps pre-process automatic weather station data from Brazil's National Institute of Meteorology (INMET). Data are available for download  at [INMET's meteorological data base portal](https://bdmep.inmet.gov.br). This script has been tested with data downloaded by request, using commas as separators. 
...
Define settings for retrieving information from INMET files.

```{r,label='inmet-instructions'}
n_inmet_head = 9L                 # Number of lines in the header
n_inmet_skip = 11L                # Number of lines to skip before data reading
n_inmet_min  = n_inmet_skip + 2L  # Minimum number of lines in a valid INMET file.

r_inmet_site  = 1L                # Row number containing site name
r_inmet_ident = 2L                # Row number containing site ID
r_inmet_lon   = 4L                # Row number containing site longitude
r_inmet_lat   = 3L                # Row number containing site latitude
r_inmet_alt   = 5L                # Row number containing site altitude
```
...
for (year in seq(from=yeara,to=yearz,by=1L)){
   ...
   for (inmet_file in inmet_file_list){
      # Before we proceed, check whether there is anything to read past the header.
      n_inmet = length(readLines(inmet_file))

      #   Check that the file has actual measurements.
      if (n_inmet >= n_inmet_min){
         # Open file and read the header
         inmet_header = suppressMessages( read_delim( file           = inmet_file
                                                    , delim          = ":"
                                                    , n_max          = n_inmet_head
                                                    , col_names      = FALSE
         ...
         inmet_header$X2 = gsub(pattern="^\\ ",replacement="",x=inmet_header$X2)
         inmet_site      = str_to_title(inmet_header$X2[r_inmet_site ])
         inmet_ident     = str_to_upper(inmet_header$X2[r_inmet_ident])
         inmet_lon       = as.numeric(gsub(pattern="\\,",replacement=".",x=inmet_header$X2[r_inmet_lon]))
         inmet_lat       = as.numeric(gsub(pattern="\\,",replacement=".",x=inmet_header$X2[r_inmet_lat]))
         inmet_alt       = as.numeric(gsub(pattern="\\,",replacement=".",x=inmet_header$X2[r_inmet_alt]))
         cat0("   - Site ",inmet_site," (",inmet_ident,").")
         ...
         this_datum = suppressMessages( read_csv2( file           = inmet_file
                                                 , skip           = n_inmet_skip
                                                 , col_names      = FALSE
                                                 , col_types      = "ccnnnnnnnnnnnnnnnnnnnnc"
                                                 , show_col_types = FALSE
                                                 )#end read_csv2
                                      )#end suppressMessages
         names(this_datum) = c( "date_str", "time_str","rain","surface_pressure","mslp"
                              , "max_surface_pressure", "min_surface_pressure"
                              , "acswdnb","t_cpu","t2m","td2m","max_t2m","min_t2m","max_td2m"
                              , "min_td2m","battery","max_rh2m","min_rh2m","rh2m","wd10","max_ws10"
                              , "ws10","dummy")
         ...
         this_datum = this_datum                              %>%
            select(when,ident,site,lon,lat,ter
                  ,surface_pressure,mslp,acswdnb,wd10,ws10,u10,v10
...
inmet_info = inmet_info                                              %>%
   ...
   mutate( n_surface_pressure = NA_integer_
         ...
         , n_td2m             = NA_integer_
         , n_rain             = NA_integer_ )                        %>%
   select(ident,lon,lat,alt,site,n_surface_pressure,n_mslp,n_acswdnb
